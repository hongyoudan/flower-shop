...
public class OperateController {
     ...
     * @param password 密码
     */
    @RequestMapping(value = "/allow/reset-password")
    public CommonResult resetPwd(String account, String authCode, String password) {
        if (StringUtils.isBlank(account) || StringUtils.isBlank(password)) {
            return CommonResult.error("账号或密码不允许为空！");
        }
        if (StringUtils.isBlank(authCode)) {
            return CommonResult.error("验证码不允许为空！");
        }
        String code = redisTemplate.opsForValue().get(account);
        if (StringUtils.isBlank(code)) {
            return CommonResult.error("验证码已过期，请重新发送~");
        }
        if (!StringUtils.equals(authCode, code)) {
            return CommonResult.error("验证码错误，请重新填写！");
        }
        Integer id = userService.selectIdByKey(account);
        if (Objects.isNull(id)) {
            return CommonResult.error("当前账号不存在");
        }
        User user = new User();
        user.setUserId(id);
        user.setPassword(SaSecureUtil.md5BySalt(password, account));
        if (BooleanUtils.isTrue(userService.updateById(user))) {
            redisTemplate.delete(account);
            return CommonResult.success("重置密码成功", user);
        }
        return CommonResult.error("重置密码失败，请稍后再试！");
    }
