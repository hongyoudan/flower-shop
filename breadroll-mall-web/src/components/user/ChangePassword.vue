  ...
  <div class="change-password center">
    <el-card class="box-card change-pwd" style="width: 90%;height: 650px;min-width: 1000px" shadow="hover">
      ...
      <div style="text-align: center;width: 750px;height: 430px;margin:auto">
        ...
        <!--表单1-->
        <el-form v-show="active===0" :model="form_0" status-icon :rules="rule_0" ref="form_0" class="demo-ruleForm"
                 style="margin:45px">
          ...
          <el-form-item label="" prop="code" style="margin:0 90px 35px;text-align: left">
            <el-input v-model="form_0.code" placeholder="请输入邮箱验证码" style="width: 73%"
                      auto-complete=“new-accounts”></el-input>
...
export default {
  ...
  methods: {
    ...
    updatePwd() {
      ...
      this.$http.post("/allow/reset-password?account=" + this.form_0.account +
        "&password=" + this.form_1.pass + "&authCode=" + this.form_0.code).then((res) => {
        loading.close();
        if (res.data.code === 200) {
          this.next()
